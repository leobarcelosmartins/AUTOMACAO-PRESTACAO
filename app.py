...
st.set_page_config(page_title="Gerador de Relatórios V0.7.9", layout="wide")
...
if modo == "Unidade (Envio de Dados)":
    ...
    st.markdown("---")
    if st.button("📦 GERAR PACOTE DE DADOS (.tatico)", type="primary", use_container_width=True):
                ...
                with tempfile.TemporaryDirectory() as tmp:
                    ...
                    with cd1:
                        with open(docx_p, "rb") as f_w:
                            st.download_button("📥 WORD (.docx)", f_w.read(), f"Relatorio_{mes_sel}.docx", use_container_width=True)
                    with cd2:
                        try:
                            ...
                            if os.path.exists(pdf_p):
                                with open(pdf_p, "rb") as f_p:
                                    st.download_button("📥 PDF", f_p.read(), f"Relatorio_{mes_sel}.pdf", use_container_width=True)
