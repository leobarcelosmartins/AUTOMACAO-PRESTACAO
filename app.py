...
st.set_page_config(page_title="Gerador de Relatórios V0.5.0", layout="wide")

# --- CUSTOM CSS PARA DASHBOARD E BOTÃO VERDE ---
st.markdown("""
    <style>
...
def excel_para_imagem(doc_template, arquivo_excel):
    """Extrai D3:E16 da aba TRANSFERENCIAS."""
    try:
...
def processar_item_lista(doc_template, item, marcador):
    """Processa um item (Bytes ou UploadedFile) para o Word."""
    ...
    try:
        ...
        st.error(f"Erro processamento {marcador}: {e}")
        return []

# --- UI ---
st.title("Automação de Relatórios - UPA Nova Cidade")
st.caption("Versão 0.5.0 - Estabilidade Garantida")

tab_manual, tab_evidencia = st.tabs(["📝 Dados de Texto", "📁 Gestão de Evidências"])

# Dicionário para capturar inputs manuais
ctx_manual = {}
            ...
            with target:
                ...
                with c_a:
                    ...
                    if pasted is not None and hasattr(pasted, 'image_data'):
                        # SEGURANÇA: Converte para bytes na hora
                        ...
                        if img_pil:
                            ...
                            st.toast(f"✅ Print salvo: {labels[m]}")
                            st.rerun()
